...
import {Link, NavLink, withRouter, Route, Switch, Redirect } from 'react-router-dom'
        ...
        return (
           <div className={style.container} style={{transform:`translateX(${isExpandSider ? '80%' : 0})`}}>
               <div className={style.header}>
                   <span className={'icon-weibiaoti12 iconfont'} onClick={this.toggleExpand}></span>
               </div>
               ...
               <Player />
               {isExpandSider && <div className={style.mask} onClick={this.toggleExpand}></div> }
           </div>
        )
    }
}
